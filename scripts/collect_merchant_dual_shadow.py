...
PLAN_SCHEMA = "controlled-merchant-dual-plan.v1"
...
def validate_merchant_dual_plan(value: object) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise ControlledEvidenceError("merchant dual plan must be an object")
    ...
    if value.get("environment") != "isolated":
        raise ControlledEvidenceError("merchant dual plan environment must be isolated")
    for field in ("batch_id", "run_id", "authorization_reference"):
        if not isinstance(value.get(field), str) or not value[field].strip():
            raise ControlledEvidenceError(f"merchant dual plan {field} is required")
    ...
    if not isinstance(scenarios, list) or not 1 <= len(scenarios) <= 3:
        raise ControlledEvidenceError(
            "merchant dual plan requires between one and three scenarios"
        )
    identities: set[tuple[str, str]] = set()
    for index, scenario in enumerate(scenarios):
        if not isinstance(scenario, dict):
            raise ControlledEvidenceError(f"merchant dual scenario {index} is invalid")
        kind = scenario.get("kind")
        case = scenario.get("case")
        identity = (str(kind), str(case))
        if kind not in MERCHANT_KINDS or case != "success":
            raise ControlledEvidenceError(
                "merchant dual plans currently accept only success scenarios"
            )
        if identity in identities:
            raise ControlledEvidenceError(
                f"duplicate merchant dual scenario {kind}/{case}"
            )
        ...
        if "legacy" in scenario:
            raise ControlledEvidenceError(
                "merchant dual plan must not carry a precomputed legacy result"
            )
    return value
...
def _legacy_projection(
    ...
    if kind == "merchant":
        if not isinstance(parsed, dict):
            raise ControlledEvidenceError("legacy merchant parser returned invalid data")
        projection["result_rows"] = [parsed]
    elif kind == "merchant_home":
        if not isinstance(parsed, list):
            raise ControlledEvidenceError("legacy merchant-home parser returned invalid data")
        projection["child_task_rows"] = parsed
    else:
        if not isinstance(parsed, list):
            raise ControlledEvidenceError(
                "legacy merchant-products parser returned invalid data"
            )
        projection["result_rows"] = parsed
    if not all(
        isinstance(row, dict)
        for rows in projection.values()
        for row in rows
    ):
        raise ControlledEvidenceError("legacy merchant projection contains invalid rows")
    return projection
...
async def _execute_scenario(
    ...
    *,
    legacy_parser: LegacyMerchantParser,
    ...
    try:
        ...
        raise ControlledEvidenceError(
            f"merchant dual scenario {kind}/success input is invalid"
        ) from exc
    identity = _digest(
        {
    ...
    item = ClaimedItem(
        ...
        options={},
        attempts=1,
        max_attempts=LEGACY_MAX_ATTEMPTS.get(kind, 5),
        lease_owner="controlled-merchant-dual",
        lease_token=f"controlled-merchant-dual-{identity}",
    )
    outcome = await plugin.execute(item)
    if not outcome.ok or outcome.result is None:
        code = outcome.failure.code if outcome.failure is not None else "unknown"
        raise ControlledEvidenceError(
            f"merchant dual scenario {kind}/success did not succeed: {code}"
        )
    if len(recording.responses) != 1:
        raise ControlledEvidenceError(
            f"merchant dual scenario {kind}/success did not capture exactly one response"
        )
    ...
    if not _is_sha256(response_sha256) or response_sha256 != captured_sha256:
        raise ControlledEvidenceError(
            f"merchant dual scenario {kind}/success response evidence does not match"
        )
    try:
        legacy_parsed = legacy_parser(
            kind,
            fetched.html,
        ...
        raise ControlledEvidenceError(
            f"current legacy parser rejected merchant scenario {kind}/success "
            f"with {type(exc).__name__}"
        ) from exc
    projection = _legacy_projection(kind, legacy_parsed)
    legacy_evidence = {
        ...
        "execution_adapter": "literal_eval_discard_debug_html"
        if kind == "merchant_products"
        else None,
    }
    legacy_evidence = {
        key: value for key, value in legacy_evidence.items() if value is not None
    }
    return {
        "kind": kind,
        "case": "success",
...
async def collect_merchant_dual_batch(
    plan: object,
    *,
    application: Application,
    legacy_parser: LegacyMerchantParser,
    ...
    plan = validate_merchant_dual_plan(plan)
    validate_runtime_prerequisites(
        application=application,
        ...
        "artifact_sha256": _digest(
            {
                "runtime": runtime,
                "response_hashes": [
                    scenario["v2"]["response_sha256"] for scenario in scenarios
                ],
            }
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch authorized merchant pages once and compare current legacy "
            "source with V2 against the same in-memory response."
        )
    )
    ...
    try:
        plan = validate_merchant_dual_plan(
        ...
        if not args.confirm_authorized_network:
            raise ControlledEvidenceError(
                "merchant dual collection requires explicit network confirmation"
            )
        legacy_source_root = args.legacy_source_root.resolve()
        legacy_parser = _load_current_legacy_merchant_parser(legacy_source_root)
        legacy_source_sha256 = _legacy_source_hash(legacy_source_root)
        with tempfile.TemporaryDirectory(prefix="amazon-crawler-merchant-dual-") as tempdir:
            settings = replace(
        ...
        message = (
            ...
            else "merchant dual shadow collection failed"
        )
        print(
            json.dumps(
