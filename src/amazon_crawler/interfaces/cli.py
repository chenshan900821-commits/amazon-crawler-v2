...
def _parser() -> argparse.ArgumentParser:
    ...
    create = sub.add_parser("create", help="create an idempotent crawl job")
    create.add_argument("inputs", nargs="*")
    create.add_argument("--kind", default="amazon.product")
    create.add_argument(
        "--input-json",
        action="append",
        default=[],
    ...
    create.add_argument("--marketplace", required=False)
    create.add_argument("--postal-code")
    create.add_argument("--mode", choices=["standard", "overseas", "realtime"], default="standard")
...
def main() -> None:
    ...
    try:
        ...
        if args.command == "init-db":
            ...
            if not all_inputs:
                raise ValidationError("create requires positional inputs or --input-json")
            job, created = app.service.create_job(
                inputs=all_inputs,
                kind=args.kind,
                ...
                options={"result_sinks": args.result_sink}
                if args.result_sink
                else None,
            )
