...
本项目是独立的技术研究与工程示例，与 Amazon 及其关联公司不存在隶属、合作、认可或担保关系；Amazon 及相关名称、商标和页面内容归其各自权利人所有。本节是项目使用边界，不构成法律意见，也不能替代使用者针对所在地区、目标站点和具体业务取得的专业审查或书面授权。

使用者在创建任何真实任务之前，必须自行确认并持续满足以下条件：

1. **先有权限，再运行。** 只访问使用者本人有权访问、或者数据权利人与站点运营方已经明确授权的页面、账号和数据。公开可见不当然代表允许自动采集、复制、建立数据库、再分发或商业使用。
2. **逐站点核对规则。** 遵守目标国家/地区的适用法律、对应 Amazon 站点与具体服务的使用条款、`robots.txt`、API 政策和数据许可；这些规则可能变化，而且不同站点、Amazon Business、联盟计划等服务可能适用不同条款。`robots.txt` 只是技术访问规则之一，遵守它本身不等于已经取得全部法律或合同许可。
3. **官方接口优先。** 能通过已经获批并覆盖该业务用途的官方接口取得数据时，优先使用 PA-API、SP-API、Amazon Ads API 或其他正式数据接口，并遵守其授权、字段用途、缓存、展示、删除和再分发限制。SP-API 的公开应用需要 Amazon 审批并由销售伙伴授权，私有应用也必须完成相应注册和自授权流程。
4. **禁止绕过与滥用。** 不得用本项目绕过登录、付费墙、验证码、访问控制、技术保护或反自动化措施；不得冒用身份、盗用或购买来历不明的账号/Cookie、隐藏来源以规避封禁，或者在收到撤销授权、`401`、`403`、`429`、验证码及明确停止信号后继续施压。本项目只检测和报告验证码，不提供破解能力。
5. **控制访问影响。** 仅采集完成已授权目的所必需的最小范围，设置审慎的频率、并发、超时、退避和任务上限；不得干扰站点稳定性、影响其他用户、制造虚假浏览/点击/流量，或将代理轮换用于规避合理限制。
6. **保护账号与秘密。** 只能使用本人或所属组织明确授权的 Cookie、Token、代理和数据库资源；不得把秘密提交到 Git、日志、截图、任务参数或 Agent 对话，不得向第三方共享会话。发现泄露后应立即停止任务、吊销凭证并清理相关数据。
7. **保护个人信息与内容权利。** 未取得合法依据、必要授权和适当安全措施时，不得采集或保存订单、账户、支付、地址、联系方式、评论者身份等个人或敏感信息。对获准数据仍应执行最小化、访问控制、加密、保存期限、删除请求和跨境传输审查；不得未经许可复制、发布、出售页面、图片、评论、商品描述或由其组成的数据库。
8. **生产和商业用途另行审批。** 本仓库中的演示、测试通过、Cookie 可用或 HTTP `200` 只证明技术链路在特定样本中工作，不证明目标用途合法、已获站点许可或适合商业化。对外服务、客户数据处理、长期监控、大规模采集和商业使用必须先完成独立的法律、合同、安全、隐私和数据许可评审。

本项目按现状提供，不承诺第三方页面持续可访问、字段完整、数据准确或适合特定用途。在适用法律允许的范围内，部署者和使用者对其配置、授权、请求行为、数据处理、下游使用及由此产生的风险负责；任何免责声明都不排除法律规定不能排除的责任。维护者可以拒绝明显越权的用例，并可在收到有效权利通知后移除相关示例或材料。

截至 2026-08-31 的参考入口：[Amazon.com Conditions of Use](https://www.amazon.com/conditionsofuse)、[Amazon.com Privacy Notice](https://www.amazon.com/privacy)、目标站点的 `/robots.txt`，以及 [Amazon SP-API 开发者注册与授权说明](https://developer-docs.amazon.com/sp-api/docs/onboarding-overview)。链接仅用于帮助使用者开始核对；实际使用时应重新查看目标站点和目标服务的最新版本。
